...
// Обратите внимание на ElementUser, NewData - компоненты, созданные в QML
   ...
   function addThisDate(id, task, desc,datebeg)
   {
       loader.writeNewInformation(id, task,desc)
       var itog = sc + 'id:"' + id + '";task:"' + task + '";desc:"' + desc + '";datebeg:"' + datebeg +'"}'
   ...
   ScrollView {
       ...
       ColumnLayout {
           id:list
           function add(id, task,desc,datebeg) {
               root.addThisDate(id, task,desc,datebeg)
               console.log(id)
               console.log(task)
               console.log(desc)
               console.log(datebeg)
